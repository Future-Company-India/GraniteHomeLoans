 ...
 * Description: Custom Elementor widgets (Broker Banner, Dynamic Rows, Icon Heading, Granite Slider, Team Members, Calculator Columns, Custom Accordion, HTML Code)
 * Version: 1.2.0
 * Author: Your Name
 */
...
function cew_register_widgets($widgets_manager)
{
  ...
  // Register HTML Code Widget
  require_once __DIR__ . "/widgets/custom-html-code-widget.php";
  $widgets_manager->register(new \Elementor_HTML_Code_Custom_Widget());
}
...
function cew_enqueue_all_styles()
{
  ...
  // HTML Code Widget CSS
  wp_enqueue_style(
    "cew-html-code-widget-css",
    plugins_url("assets/css/custom-html-code-widget.css", __FILE__),
    [],
    DEPLOYMENT_VERSION
  );
}
...
function cew_add_widget_categories($elements_manager)
{
  ...
  $elements_manager->add_category("broker-elements", [
    "title" => esc_html__("Broker Elements", "cew"),
    "icon" => "fa fa-plug",
  ]);

  // Custom Elements Category
  $elements_manager->add_category("custom-elements", [
